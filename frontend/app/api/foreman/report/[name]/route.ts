...
export async function GET(_: Request, { params }: { params: Promise<{ name: string }> }) {
  ...
  const res = await fetch(
    `${base}/api/method/olimp_construction.api.foreman.get_report?name=${encodeURIComponent(name)}`,
...
export async function PATCH(request: Request, { params }: { params: Promise<{ name: string }> }) {
  ...
  const res = await fetch(
    `${base}/api/method/olimp_construction.api.foreman.set_report_status`,
    {
      method: "POST",
