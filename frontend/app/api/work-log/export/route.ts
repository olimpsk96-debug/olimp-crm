...
const METHODS: Record<string, { method: string; mime: string; ext: string }> = {
  pdf: {
    method: "olimp_construction.api.exports.worklog_pdf",
    mime: "application/pdf",
    ext: "pdf",
  },
  xlsx: {
    method: "olimp_construction.api.exports.worklog_excel",
...
export async function GET(request: Request) {
  ...
  if (!res.ok) {
    ...
    return NextResponse.json(
      { error: `Frappe: ${res.status}`, details: text },
      { status: res.status }
    );
  }
  ...
  return new NextResponse(buf, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="KS-6_${name}.${spec.ext}"`,
