...
### 2026-05-17 — scheduler/queue_short/queue_long падают с exit 127 после compose recreate (РАЗВЁРНУТЫЙ ФИКС)
Симптом повторяется регулярно (зафиксирован 2026-05-13, 2026-05-14 и ещё раз 2026-05-17): после любого `docker compose stop/up` или `compose up -d --force-recreate` для backend — сервисы scheduler/queue_short/queue_long падают с exit 127 («command not found»). В предыдущих случаях временное лечение было `docker start <name>` (поднимает прежний инстанс с сохранённым CMD), но это не решало корневую проблему.

**Корень:** воркеры используют stock entrypoint `/usr/local/bin/entrypoint.sh` (linkует assets и делает `exec "$@"`) + command `["bench", "schedule"]`. Backend имеет custom entrypoint `docker-entrypoint-backend.sh` который выполняет `pip install -r requirements.txt` перед `exec`. Воркеры — нет. При первом старте после recreate `bench worker/schedule` стартует ДО того как зависимости из `apps/olimp_construction/requirements.txt` установлены (pyarrow, pandas, rapidfuzz, anthropic, openai, qdrant-client) → bench падает с ImportError при импорте app → код выхода 127. Restart-policy `unless-stopped` теряет попытки и контейнер уходит в Exited.

**Решение (v6.10.1):** для всех трёх воркеров в docker-compose.yml:
1. `entrypoint: ["/scripts/docker-entrypoint-backend.sh"]` — переиспользуем backend-entrypoint (он делает pip install + exec)
2. `command: ["/usr/local/bin/bench", "worker", ...]` — абсолютный путь к bench (PATH-агностично)
3. `volumes: + ./scripts/docker-entrypoint-backend.sh:/scripts/docker-entrypoint-backend.sh:ro`

После применения: `docker compose up -d --force-recreate queue-short queue-long scheduler` → все три Up стабильно, в логах «Устанавливаем зависимости olimp_construction... Зависимости установлены.» → bench работает без ошибок.

**Правило:** любой контейнер на основе `frappe/erpnext:v15` который импортирует наш app olimp_construction должен использовать custom-entrypoint с pip install. Stock entrypoint этого не делает.
...
### 2026-05-17 — DocType с Link на ещё не созданный custom DocType → WrongOptionsDoctypeLinkError
При создании v6.3 (5 DocType из OLIMP.STACK) первая попытка batch-create в порядке `[boq_section, boq_position, assembly_item, construction_assembly, boq]` упала: `BOQ Position: Options must be a valid DocType for field Сборка in row 4` — потому что `BOQ Position` имеет Link → Construction Assembly, который ещё не создан. **Решение:** правильный порядок зависимостей — `[boq_section (без deps), assembly_item (без deps, Catalog Resource уже есть), construction_assembly (uses Assembly Item), boq_position (uses Construction Assembly), boq (uses всё выше)]`. **Правило:** при batch-создании custom DocType через console построй граф зависимостей по Link-полям и делай topological sort. Frappe не разрешит ссылаться на несуществующий DocType, даже если оба создаются в одной транзакции.

### 2026-05-17 — Frappe Data field max 140 chars — JWT и base64-картинки не помещаются
Создал `Construction Proposal` с полем `share_token: Data` для JWT. При первом `doc.save()` после `generate_share_token()` упало с `CharacterLengthExceededError: '<strong>Share token (JWT)</strong>' (eyJhbG…) will get truncated, as max characters allowed is 140`. JWT HS256 даже с минимальным payload занимает 150-200 символов. То же касается base64 PNG-подписей (десятки тысяч символов). **Решение:** для токенов использовать `Small Text` (TEXT, до 65K), для подписей — `Long Text` (LONGTEXT, до 4GB). При смене типа уже созданного DocType: (1) исправить .json, (2) обновить metadata через `frappe.get_doc("DocType", X).save()`, (3) `ALTER TABLE \`tabX\` MODIFY COLUMN field TEXT/LONGTEXT NULL`. **Правило:** при выборе fieldtype для нового поля ВСЕГДА оценить максимальную длину контента. Data (140) — только для коротких строк (имена, номера). JSON, токены, URL длинных файлов, base64-данные — Small Text или Long Text.

### 2026-05-17 — `%` в `frappe.db.sql` без параметра → `ValueError: unsupported format character`
В `api/capacity.get_heatmap` была строка `CONCAT(project, ' (', ROUND(allocated_pct), '%)')` для генерации `"PR-001 (80%)"` в GROUP_CONCAT. pymysql парсит `%` как placeholder и падает с `ValueError: unsupported format character ')' (0x29) at index 225`. **Решение:** передавать `%` через параметризацию: `... CONCAT(..., %(pct_sign)s)`, params={..., "pct_sign": "%)"}`. Альтернативно — экранировать `%%`, но это создаёт хрупкость. **Правило:** ЛЮБОЙ литерал `%` внутри SQL-строки `frappe.db.sql()` должен идти через параметр; даже в строках вроде `LIKE 'X%'` использовать `LIKE %(pattern)s` где `pattern = "X%"`. То же для `printf`-style форматирования pymysql.
...
| **v6.10.1** | **2026-05-17** | **Infra-fix: scheduler/queue_short/queue_long больше не падают с exit 127 после compose recreate. Корень: воркеры использовали stock entrypoint stock-образа `frappe/erpnext:v15` (только linkует assets и `exec`), но НЕ устанавливали `pip install -r apps/olimp_construction/requirements.txt` перед стартом → `bench worker` падал на ImportError (pyarrow/pandas/rapidfuzz/anthropic/openai/qdrant-client недоступны). Решение: всем трём воркерам в docker-compose.yml установлен `entrypoint: /scripts/docker-entrypoint-backend.sh` (тот же что у backend — pip install + exec) + абсолютный путь `/usr/local/bin/bench` в command (PATH-агностично) + mount entrypoint-скрипта. После `docker compose up -d --force-recreate queue-short queue-long scheduler` — все три Up стабильно, в логах «Зависимости установлены.» Закрывает повторяющийся блокер (баги 2026-05-13, 2026-05-14, 2026-05-17). + новая запись §18 «scheduler/queue падают с exit 127 (РАЗВЁРНУТЫЙ ФИКС)».** |
| **v6.10** | **2026-05-17** | **AI Cost Advisor в карточке тендера. Закрывает цикл «тендер → BOQ в один клик». **Backend** `boq_advisor.save_generated_boq` принимает опциональный `tender` параметр — записывается в `notes` BOQ как «Источник: тендер TENDER-XXX» (выделенного Link-поля BOQ↔Tender нет, notes-связь достаточна для аудита). `/api/boqs/ai/route.ts` пробрасывает `tender` в save. **Frontend** новый компонент `<AiBoqBlock>` в `TenderDrawer.tsx` (фиолетовый блок ниже AI-оценки): кнопка «Сгенерировать смету» → textarea с предзаполненным контекстом (title + work_type + region + notes тендера) → «🤖 Сгенерировать» → preview-карточка (название/summary/разделы/позиции/сборки + сумма прямых затрат в млн ₽) → «✓ Сохранить как BOQ» (передаёт customer тендера, project_link если есть, tender=tender.name) → success-блок с BOQ-{name} и ссылкой на /boqs. Smoke: backend `save_generated_boq` с tender param создаёт BOQ-2026-00076 с корректным notes «Источник: тендер TENDER-TEST-001», TS 0 ошибок.** |
| **v6.9.1** | **2026-05-17** | **Prompt caching в boq_advisor (CLAUDE.md §5.2 «обязательно для system prompt»). System разбит на массив блоков: `[SYSTEM_PROMPT, assemblies_ctx]` с `cache_control: {"type": "ephemeral"}` на последнем — кэшируется всё до него. assemblies_ctx перенесён из user в system (ассемблеи меняются раз в неделю, отлично для кэша 5 мин). Возврат `cache_read_input_tokens` и `cache_creation_input_tokens` в tokens_used. **Замечание:** при текущем размере (system ~800 токенов) Haiku 4.5 не активирует кэш (минимум ~2048) — `cache_read=0` и `cache_write=0` на тестах. По мере роста каталога Construction Assembly (>30-40 сборок) кэш заработает автоматически и сэкономит до 90% input-токенов на повторных вызовах. Код корректен и production-ready.** |
| **v6.9** | **2026-05-17** | **AI Cost Advisor — Claude генерирует BOQ из описания работ. **Backend** `api/ai/boq_advisor.py`: `generate_boq(description, project, customer, region)` — Claude Haiku 4.5 с системным промптом «главный инженер ОЛИМПа» + контекст из 50 Construction Assemblies (assembly_code/name/category/unit/market_rate/labor_hours) для матчинга → строгий JSON c sections[]/positions[]/title/summary, валидация структуры, авто-маркировка валидных assembly_code (невалидные → null), подсчёт direct_cost. `save_generated_boq(title, sections, positions, summary, project, customer, overhead/profit/contingency/vat)` создаёт BOQ + дочерние BOQ Section/Position с авто-итогами (накладные 8% + прибыль 15% + резерв 5% + НДС 20%). `_extract_json` снимает markdown-обрамление если Claude добавил. Frappe `frappe.has_permission("BOQ", "create")` для обеих функций. **Frontend** `/api/boqs/ai/route.ts` (runtime=nodejs, maxDuration=60 — Claude думает 20-40сек) action-map generate/save → proxy на Frappe. Страница `/boqs` — `CreateFromEstimateDialog` стал 2-табовым: «📋 Из существующей сметы» (старый workflow) + «🤖 AI из описания работ» (textarea ≥20 символов → «🤖 Сгенерировать» → preview-таблица с title/summary/sections/positions/direct_cost/matched_assemblies/tokens, бейджи assembly_code зелёным → «✓ Создать BOQ» сохраняет). Smoke: «Фундаментная плита 7×7×2м В25 F150 W6, двойное армирование 30 кг/м³, 32 анкера М30 L=600мм» → 4 раздела, 10 позиций, 1.67М₽ direct, 2.56М₽ grand_total, BOQ-2026-00075 сохранён, токены IN=1455 OUT=1469.** |
| **v6.8** | **2026-05-17** | **BOQ Won → Construction Project + Schedule Tasks (закрытие workflow). Завершён обещанный из `docs/01-architecture.md` workflow «выиграли тендер → проект в исполнение». Без n8n — напрямую в Frappe. **Backend** `api/boqs.create_project_from_boq(name, start_date, planned_end_date, foreman)`: проверка status=Won и customer заполнен → создание Construction Project (title/customer/contract_amount=grand_total/planned_cost=direct_cost/planned_margin_pct/foreman/oce_boq_id для двусторонней связи) → автоматическое создание Schedule Tasks по одной на BOQ Section с равномерным распределением сроков (start=now, end через 90 дней по умолчанию, делится на N разделов) + cost_summary в notes каждой задачи + Telegram-уведомление директору с маржой и кол-вом задач. Idempotency через проверку Construction Project.oce_boq_id — повторный вызов возвращает existing с `skipped: "already_exists"`. **Frontend:** в `/boqs` для статусов Won/Submitted добавлена кнопка 🏗 (фиолетовая) — confirm-диалог → если не Won, предлагает сменить статус → создаёт проект → toast с маржой и количеством задач. Smoke: BOQ-2026-00071 (3 секции, 101,376 ₽) → PR-2026-0002 (маржа 34.9%, 3 Schedule Tasks по 30 дней каждая, foreman заполнен, oce_boq_id связан), idempotency works. + новая запись р.18: `tabSeries` для `PR-{YYYY}-` снова застрял (был баг из v2.5) — нужен SQL-фикс через `frappe.db.set_value("Series", "PR-2026-", "current", MAX_NUMBER)`.** |
| **v6.7** | **2026-05-17** | **Ресурсный план — закрытие placeholder из v6.2.2 (последнего из Three-tab UX). Tab «Ресурсы» в `/estimates/[name]/grid` через `<ResourcePlan>`. **Backend** `api/estimate_grid.get_resource_breakdown(estimate)`: пробегает позиции сметы, по `item_code` находит Construction Assembly, разворачивает Assembly Items × qty позиции, агрегирует по 4 типам (material/labor/equipment/subcontract), считает totals + unmapped (позиции без сборки), собирает `from_items` (из каких позиций сметы взялся ресурс — top-3 уникальных). **Frontend:** 5 KPI сверху (📦 Материалы / 👷 Труд / 🚜 Оборудование / 🤝 Субподряд / ❓ Без разбивки) с цветовой кодировкой и счётчиками, гранд-тотал с акцентом, 4 цветные таблицы по группам ресурсов (отсортированы по amount DESC) — каждая строка с описанием/кол-вом/ценой/суммой и колонкой «Из позиций» (показывает первые 2 названия). Секция «Позиции без сборки» с подсказкой как получить разбивку (применить «⊞ + Сборка»). Smoke: для EST-2026-00009 после Apply AKZ-G3-M2 × 120 м² корректно: material 24,948 ₽ (Эмаль ХС-720 42 кг + Грунт ВЛ-02 21.6 кг), labor 18,240 ₽ (Маляр АКЗ 48 ч), unmapped 12,000 ₽. **Workflow полностью замкнут:** Обмеры → Работы (с применением сборок) → Ресурсы (разбивка) → КП → Подпись клиента → BOQ.** |
| **v6.6** | **2026-05-17** | **Обмерный лист — закрытие placeholder из v6.2.1. Реализован Tab «Обмеры» в `/estimates/[name]/grid` через компонент `<MeasurementSheet>`. **Frontend-only** (без нового DocType): локальный state, авто-сохранение в localStorage per-estimate (`olimp-measurements-{name}`). Таблица с inline-edit: Что обмеряем / Ед. / Формула / Длина / Ширина / Высота / n / Итого / ×. **6 типов формул**: L×W (площадь), L×W×H (объём), L×W×n (площадь × кол-во), L×W×H×n (объём × кол-во), L×n (длина × кол-во), manual (вручную). Неактивные поля затухают (opacity 0.4) в зависимости от формулы — пользователь сразу видит что заполнять. Авто-подсветка строк с total>0 зелёным. Сводка Σ по единицам (м²/м³/т) сверху, кнопка «🗑 Очистить» и «✓ Применить в смету» с подсветкой количества валидных строк. **Backend** `api/estimate_grid.apply_measurements(estimate, rows)`: создаёт раздел-метку «📐 Обмеры (N позиций)» + N позиций в Estimate.items с qty=total, описанием формулы в notes («Формула: 7 × 7 × 2 = 98 · 7×7×2, n=1»), без цены (пользователь добавит через «⊞ + Сборка»). Smoke 4/4: применил 3 обмера (плита 49 м², бетон 98 м³, армокаркасы 2.94 т вручную) → 4 items добавлены в EST-2026-00009.** |
| **v6.5** | **2026-05-17** | **Apply Assembly в EstimateGrid + BOQ frontend page. (1) **Кнопка «⊞ + Сборка»** в Excel-редакторе сметы (`/estimates/[name]/grid`) → модалка `<ApplyAssemblyModal>` с поиском по 8+ сборкам, category-filter, live-preview итога (market_rate × quantity + markup_pct), input `quantity` (с автофокусом), `markup_pct`, кнопка «✓ Применить» → создаёт раздел-метку + позицию в смете с work_type-маппингом → toast с суммой. Закрывается ESC. (2) **BOQ frontend** — `api/boqs.py` с list/get_summary/create_from_estimate (snapshot позиций Estimate → BOQ Section/Position с автонумерацией 1.1/1.2/2.1, авто-итоги накладные 8% + прибыль 15% + резерв 5% + НДС 20%) + recalculate_totals + change_status (Telegram при Won) + delete. Страница `/boqs` со списком + 5 KPI (всего/в работе/выиграно/конверсия/сумма выигранных) + статус-фильтр + inline-select смены статуса + ссылка на Frappe-админку `/app/boq/{name}` для полного редактирования + диалог «Создать BOQ из сметы» с выбором Estimate. **Намеренно лёгкая страница** — полное редактирование разделов/позиций живёт в Frappe-админке (там готовый child-table UI), фронт даёт обзор + workflow. Smoke 6/6 endpoints, create_from_estimate EST-2026-00009 → BOQ-2026-00067 (2 секции, 2 позиции, direct=66K, grand_total=101K с НДС), change_status работает, TS 0 ошибок, HTTP 200 на `/boqs`.** |
| **v6.4** | **2026-05-17** | **Construction Assembly UI + 8 типовых сборок Олимпа. После v6.3 (BOQ-DocType без UI) добавлен фронт. `api/assemblies.py`: list/get/save/delete + get_categories + seed_olimp_assemblies (8 реальных сборок Олимпа: АКЗ-G3-M2 / АКЗ-G5-M2 / CFRP-LAYER-M2 / METAL-MONT-T / ROOFING-NAPL-M2 / PROMALP-SHIFT / BETON-V25-M3 / FIRE-R90-M2 с реальными ресурсами: грунт ВЛ-02, эмаль ХС-720, ламель CarboDur S, бетон В25 F150 W6) + apply_to_estimate (вставляет сборку × quantity в Estimate с work_type-маппингом АКЗ→АКЗ, Бетон→Монолит, Углеволокно→Усиление). Авто-calc base_rate из items, margin_percent = (market-base)/base × 100. Страница `/assemblies` — grid карточек с code/name/category/база/рынок/маржа (цветовая кодировка ≥30% зелёный / 15-30% жёлтый / <15% красный), category-chips для фильтра, поиск, кнопка «🌱 Установить 8 типовых». Drawer редактирования с inline-таблицей ресурсов (resource_type: material/labor/equipment/subcontract color-coded), live-preview база/рынок/маржа. Smoke 6/6 endpoints, 8 сборок засеяны с маржами 30-80%, apply AKZ-G3-M2 × 120 м² = 54,000 ₽. + новая запись р.18: Estimate Item.item_name обязателен даже для is_section=1 (нужно дублировать section_title в item_name); work_type Estimate Item имеет фикс-набор (АКЗ/Кровля/Промальп/Монолит/Усиление/Комплексный) — категории Assembly надо мапить.** |
| **v6.3** | **2026-05-17** | **OLIMP.STACK integration (мини-план из updete.zip) — BOQ + Construction Assembly + 31 Custom Fields. После анализа архива (полный план OCE-интеграции с +4 ГБ RAM, OpenAI, AGPL-инфра-нагрузкой) реализован мини-план: только DocType + Custom Fields внутри olimp-erp, без подъёма OCE-контейнера. **5 новых DocType:** (1) `BOQ` (autoname `BOQ-{YYYY}-{####}`) — главная сущность сметы с разделами/позициями/накладными/прибылью/НДС, версионирование, oce_boq_id для будущей синхронизации; (2) `BOQ Section` (child) — иерархия разделов; (3) `BOQ Position` (child) — позиции с привязкой к Construction Assembly + Catalog Resource CWICR; (4) `Construction Assembly` (autoname=assembly_code, типовые сборки АКЗ-G3-M2 и т.п.); (5) `Assembly Item` (child). **31 Custom Field** через `install_oce.sync_oce_fields()`: Construction Project (9: oce_project_id/boq_id/spi/cpi/eac), Task (9: dependency_type FS/FF/SS/SF, lag_days, baseline_start/end, earned_value), Project Risk (10: Monte Carlo — probability_value, impact_cost_min-max, monte_carlo_p80), Change Order (3: doc_link_minio, oce_change_order_id). **Item Groups** (13): Бетон/Металлопрокат/АКЗ/Углеволокно/Кровельные/Метизы/ИТР/Рабочие/Промальп/Сварщики/Грузоподъёмная/Сварочное/АКЗ-оборудование под «Строительство (ОЛИМП)». **n8n workflows** скопированы в `n8n/workflows/oce/` (импорт при подъёме OCE). **Документация** в `docs/olimp-stack/` (6 файлов + STATUS.md с навигатором). **Deploy-конфиги OCE** в `deploy/oce/` (compose, Caddyfile, postgres-init, env-template, import-shell) — не активированы. Smoke 5/5: создание Assembly + BOQ + кастомные поля + 13 Item Groups + cleanup OK. + новая запись р.18: при создании DocType с Link на ещё не созданный другой кастомный DocType — `WrongOptionsDoctypeLinkError` → порядок: сначала dependencies, потом parent.** |
| **v6.2** | **2026-05-17** | **Фаза 2 редизайна: Smart Estimate UX (RevoGrid + Three-tab + Toggle к КП). **Backend** `api/estimate_grid.py`: (1) `get_items(estimate)` плоский список позиций + totals (base/our/margin); (2) `save_items_batch(estimate, rows)` — snapshot-режим (полная замена items сметы, родитель пересчитывает margin); (3) `bulk_apply_markup(estimate, markup_pct)` — массовая наценка ко всем позициям с пересчётом our_unit_price; (4) `create_proposal_from_estimate(estimate, title, template)` — Procore Proposal Builder: автогенерирует Construction Proposal с TipTap JSON, в котором смета разложена по разделам в виде TipTap-таблиц + добавлен дефолтный график оплаты + merge-теги для итога. **Frontend:** (1) Страница `/estimates/[name]/grid` — Excel-like редактор смет на `@revolist/react-datagrid` (MIT). 12 колонок (№/Раздел/Код/Наименование/Ед./Кол-во/Цена база/Сумма база/Наша цена/Наша сумма/Тип/Заметки) с inline-редактированием, pin первой колонки, range-selection, auto-recalc amounts по qty×price. Custom cell templates: `§` для разделов (фиолетовые), числа выровнены вправо, sum-колонки read-only с серым/зелёным фоном. (2) Three-tab UX: Работы (активна) / Обмеры (placeholder v6.2.1) / Ресурсы (placeholder v6.2.2). (3) 4 KPI сверху (база/наша/маржа ₽/маржа %) с цветовой кодировкой (зелёный >15%, жёлтый 5-15%, красный <5%). (4) Кнопки: 💾 Сохранить (Ctrl+S, авто-batch), 📈 Наценка ко всем (prompt %), 📋 Создать КП (создаёт Construction Proposal и переходит туда). (5) Кнопка «⊞ Открыть в Excel-редакторе» в EstimateDrawer для перехода из обычной сметы в spreadsheet-вид. Установлен `@revolist/react-datagrid@latest`. Smoke 4/4 endpoints, TS 0 ошибок (через `unknown[]` для columns + index signature на GridRow), HTTP 200 на `/estimates/EST-2026-00009/grid`, batch-save 1→2 строки + auto-recalc маржи работают.** |
| **v6.1** | **2026-05-17** | **Фаза 1.5: PDF/DOCX экспорт КП + Univer spreadsheet-блок. **Backend** `api/proposals_export.py`: (1) полноценный TipTap JSON → HTML walker (поддержка heading/paragraph/text/marks/lists/blockquote/table/image + кастомные `MergeTag` с резолвом `{{customer.name}}`/`{{project.title}}`/`{{estimate.total}}` + `PaymentSchedule` с автоматическим расчётом сумм + `Spreadsheet` с парсингом Univer snapshot в HTML-таблицу); (2) `export_pdf(name)` через WeasyPrint с Apple-style CSS (A4 18×16mm margins, Helvetica Neue, header/summary/footer/sign-block); (3) `export_docx(name)` через python-docx с программным walker (paragraphs/headings/lists/tables/payment-schedule), info-table заказчик/сумма/срок, секция подписи если есть, footer; (4) `preview_html(name)` для модалки превью. Добавлен `python-docx>=1.2` в requirements.txt. **Frontend:** Next.js route `/api/proposals/[name]/export?format=pdf|docx` с proxy к Frappe + правильным Content-Disposition. Кнопки 📄 PDF и 📝 DOCX в шапке редактора КП — авто-сохранение dirty перед экспортом + открытие в новой вкладке. **Univer Sheets блок:** новый TipTap node `SpreadsheetNode` (block atom, draggable) + компонент `<UniverSheet>` (грузится через dynamic ssr:false, инициализирует Univer с `UniverSheetsCorePreset({header,toolbar,formulaBar})` + русская локаль, 400+ Excel-формул, debounced save snapshot в node.attrs). Дефолтный workbook с шапкой «Наименование/Ед./Кол-во/Цена/Сумма» и примером работы с формулой `=C2*D2`. Кнопка «⊞ Калькуляция» в тулбаре редактора (фиолетовая, отличается от обычной таблицы). Установлены `@univerjs/preset-sheets-core@0.22`. Smoke: PDF 12-17 KB валидный, DOCX 37 KB ZIP-валидный, Univer snapshot корректно парсится в HTML/DOCX. + новая запись р.18: Univer 0.5 не поддерживает React 19 — нужна версия 0.22+. + Pro-Tip: `runtime = "nodejs"` обязателен для route handler с PDF (Edge runtime ломает fontkit).** |
| **v6.0** | **2026-05-17** | **Фаза 1 редизайна: КП-конструктор (Procore Proposal Builder + PandaDoc). После разведки 3 параллельных subagents (топ open-source CRM, строительные сервисы, frontend-архитектура) внедрена основа КП-конструктора. **Backend:** (1) DocType `Construction Proposal` (autoname `КП-{YYYY}-{####}`) с TipTap JSON в content_json, JWT share-token (HS256, секрет в site_config), полями для подписи клиента (signed_by_name/_ip/signature_data_url), счётчиком просмотров. (2) DocType `Proposal Template` (autoname=template_id) — шаблоны КП с default_content_json. (3) `api/proposals.py` 15 endpoints: list/get/save/delete + list_templates/get_template/save_template + get_merge_data (резолвит `{{customer.name}}`/`{{project.title}}`/`{{estimate.total}}` из связанных сущностей) + generate_share_token + render_for_public (allow_guest=True, инкремент view_count) + submit_signature (allow_guest=True, Telegram-уведомление директору) + mark_sent + get_summary. **Frontend:** (1) TipTap 2.10 редактор с тулбаром (B/I/S, H1-3, списки, таблицы, undo/redo) — `<ProposalEditor>` в `components/proposal/`. (2) Кастомные nodes: `MergeTag` (inline atom — отображает `{{customer.name}}` зелёным когда резолвится из window.__mergeData, синим plain placeholder когда нет) и `PaymentSchedule` (block atom — таблица «Этап / % / Срок дн. / Сумма ₽» с inline-редактированием, авто-расчётом сумм от proposal.total_amount, валидацией Σ%=100). (3) Страница `/proposals` с 5 KPI (всего/в работе/согласовано/конверсия/сумма) + таблицей + CreateProposalDialog с выбором шаблона. (4) Страница `/proposals/[name]` — split-layout: TipTap editor + sidebar с параметрами/share-token статусом/кнопками «Сохранить» (Ctrl+S) + «Поделиться» + «Отметить отправленным». (5) Публичная страница `/p/[token]` без авторизации — Apple-style design (white card, no sidebar), read-only TipTap render, react-signature-canvas для подписи пальцем/мышью, JWT-валидация через render_for_public, success-screen после подписания. Установлено 278 npm-пакетов: `@tiptap/react@2.10` + 7 extensions + `@univerjs/presets@0.22` (для Phase v6.1 spreadsheet-блок) + `react-signature-canvas@1.0.6` + `@types/react-signature-canvas`. Установка попробовала Univer 0.5 — конфликт с React 19, обновили до 0.22. + новая запись р.18: Frappe Data fields max 140 chars — для JWT (180+ chars) нужен Small Text/Long Text + миграция через ALTER TABLE MODIFY COLUMN. Smoke: 8/8 endpoints зелёные, TS 0 ошибок, HTTP 200 на `/proposals` и `/p/[token]`, end-to-end сценарий «создать → share-link → клиент открыл → подписал» работает.** |
| **v5.6** | **2026-05-17** | **Capacity Planning по бригадам (Asana-style). DocType `Crew Allocation` (autoname `CA-{YYYY}-{####}`): бригада × проект × неделя → загрузка %, плановые часы, кол-во рабочих. `api/capacity.py` 5 endpoints (list/save/delete/get_crews/get_heatmap). `get_heatmap(from_week, weeks)` строит сетку с агрегацией allocated_pct по неделям + находит перегруз (>100%) и простой (<50%). Страница `/capacity` — heatmap-таблица с цветовой кодировкой (простой→серый / норма→зелёный / уплотнённо→жёлтый / перегруз→красный), sticky-колонка бригад при скролле, средняя загрузка по неделям в подзаголовке. Клик на ячейке → drawer с проектами этой бригада-недели + редактирование. Внизу — 2 warning-блока: ⚠ Перегрузка и 💤 Простой. Drawer создания с datalist suggestions по существующим бригадам. + новая запись р.18: `%` в `frappe.db.sql` строке без параметризации даёт `ValueError: unsupported format character ')' at index N` — символ % надо передавать через `%(param)s`. Smoke 5/5 endpoints, TS 0 ошибок, HTTP 200 на `/capacity`.** |
| **v5.5** | **2026-05-17** | **Tender Guarantee — обеспечения 44/223-ФЗ (СБИС Госзакупки / Tenderok). DocType `Tender Guarantee` (autoname `TG-{YYYY}-{####}`) с 5 типами: Обеспечение заявки / Обеспечение контракта / Банковская гарантия / СРО взнос / Гарантийные обязательства. Для БГ — отдельная секция с банк-гарантом, % комиссии, авто-расчётом стоимости БГ. Статусы Активна/Истекла/Возвращена/Использована/Аннулирована. `api/guarantees.py` 4 endpoints + cron `check_guarantee_expiry` (daily): алерты в Telegram за 30/14/7/3/1 день до истечения с защитой от спама (next_reminder_sent, не чаще раза в 7 дней) + авто-перевод просроченных в «Истекла». Страница `/guarantees` с 4 KPI (заморожено в обеспечениях / комиссии банкам / активных / истекают в 30 дней), отдельный warning-блок «Скоро истекают» сверху, таблица с цветовой кодировкой по сроку (🔴/🟠/🟡/🟢), drawer с условным показом банковской секции. Smoke 5/5 endpoints, TS 0 ошибок, HTTP 200 на `/guarantees`.** |
